...
//		the rx buffer index starts at half way through the buffer and goes to the end
		for (int i = 0; i < txCount; ++i) {
			...
			yi[j] = (1.2898592677e-06) * aRxBuffer[j] + (5.1594370709e-06) * aRxBuffer[j-1] \
			+ (7.7391556064e-06) * aRxBuffer[j-2] + (5.1594370709e-06) * aRxBuffer[j-3] \
			+ (1.2898592677e-06) * aRxBuffer[j-4] \
			- (-3.8199513482e+00) * yi[j-1] - (5.4758859329e+00) * yi[j-2] \
			- (-3.4910942344e+00) * yi[j-3] - (8.3518028741e-01) * yi[j-4];
			++j;
			yi[j] = (1.2898592677e-06) * aRxBuffer[j] + (5.1594370709e-06) * aRxBuffer[j-1] \
			+ (7.7391556064e-06) * aRxBuffer[j-2] + (5.1594370709e-06) * aRxBuffer[j-3] \
			+ (1.2898592677e-06) * aRxBuffer[j-4] \
